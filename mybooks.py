...
def add_book():
    ...
    title_entry.delete(0,'end') # clear input after inserting
    author_entry.delete(0,'end')
    isbn_entry.delete(0,'end')
    con.commit()
...
isbn_label = ttk.Label(root, text="ISBN", background="blue", font=("TkDefaultFont", 14))
...
# Add a button to insert input into db
add_btn = Button(root, text="Add Book" ,bg="green",fg="white", font="helvectica 10 bold", command=add_book)
add_btn.grid(row=0, column=6, sticky=W)

list_bx = Listbox(root, height=16,width=40,font="helvetica 13", bg="sky blue")
...
view_btn = Button(root, text="View all records" ,bg="black",fg="white", font="helvectica 10 bold", command=view_records)
view_btn.grid(row=15, column=1)

clear_btn = Button(root, text="Clear Screen" ,bg="maroon",fg="white", font="helvectica 10 bold", command=clear_screen)
clear_btn.grid(row=15, column=2)

modify_btn = Button(root, text="Modify Record" ,bg="purple",fg="white", font="helvectica 10 bold", command=update_records)
modify_btn.grid(row=15, column=3)

delete_btn = Button(root, text="Delete Record" ,bg="red",fg="white", font="helvectica 10 bold", command=delete_records)
delete_btn.grid(row=15, column=4)

exit_btn = Button(root, text="Exit Application" ,bg="green",fg="white", font="helvectica 10 bold", command=root.destroy)
